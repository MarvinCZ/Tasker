 ...
 *
 * @method     ChildGroupQuery leftJoinShared($relationAlias = null) Adds a LEFT JOIN clause to the query using the Shared relation
 * @method     ChildGroupQuery rightJoinShared($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Shared relation
 * @method     ChildGroupQuery innerJoinShared($relationAlias = null) Adds a INNER JOIN clause to the query using the Shared relation
 *
 * @method     ChildGroupQuery joinWithShared($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Shared relation
 *
 * @method     ChildGroupQuery leftJoinWithShared() Adds a LEFT JOIN clause and with to the query using the Shared relation
 * @method     ChildGroupQuery rightJoinWithShared() Adds a RIGHT JOIN clause and with to the query using the Shared relation
 * @method     ChildGroupQuery innerJoinWithShared() Adds a INNER JOIN clause and with to the query using the Shared relation
 *
 * @method     \Models\UserGroupQuery|\Models\SharedQuery endUse() Finalizes a secondary criteria and merges it with its primary Criteria
 *
...
abstract class GroupQuery extends ModelCriteria
{
    ...
    public function filterByShared($shared, $comparison = null)
    {
        if ($shared instanceof \Models\Shared) {
            return $this
                ->where("'group' = ?", $shared->getToType(), 2)
                ->addUsingAlias(GroupTableMap::COL_ID, $shared->getToId(), $comparison);
        } else {
