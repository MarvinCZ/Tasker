 ...
 * @method     ChildFileQuery orderByNoteId($order = Criteria::ASC) Order by the note_id column
 ...
 * @method     ChildFileQuery groupByNoteId() Group by the note_id column
 ...
 *
 * @method     ChildFileQuery leftJoinNote($relationAlias = null) Adds a LEFT JOIN clause to the query using the Note relation
 * @method     ChildFileQuery rightJoinNote($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Note relation
 * @method     ChildFileQuery innerJoinNote($relationAlias = null) Adds a INNER JOIN clause to the query using the Note relation
 *
 * @method     ChildFileQuery joinWithNote($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Note relation
 *
 * @method     ChildFileQuery leftJoinWithNote() Adds a LEFT JOIN clause and with to the query using the Note relation
 * @method     ChildFileQuery rightJoinWithNote() Adds a RIGHT JOIN clause and with to the query using the Note relation
 * @method     ChildFileQuery innerJoinWithNote() Adds a INNER JOIN clause and with to the query using the Note relation
 *
 ...
 * @method     ChildFile findOneByNoteId(int $note_id) Return the first ChildFile filtered by the note_id column
 ...
 * @method     ChildFile requireOneByNoteId(int $note_id) Return the first ChildFile filtered by the note_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildFile[]|ObjectCollection findByNoteId(int $note_id) Return ChildFile objects filtered by the note_id column
...
abstract class FileQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, note_id, path, created_at, updated_at FROM file WHERE id = :p0';
        try {
    ...
    public function filterByNoteId($noteId = null, $comparison = null)
    {
        if (is_array($noteId)) {
            $useMinMax = false;
            if (isset($noteId['min'])) {
                $this->addUsingAlias(FileTableMap::COL_NOTE_ID, $noteId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($noteId['max'])) {
                $this->addUsingAlias(FileTableMap::COL_NOTE_ID, $noteId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(FileTableMap::COL_NOTE_ID, $noteId, $comparison);
    }
    ...
    public function filterByNote($note, $comparison = null)
    {
        if ($note instanceof \Models\Note) {
            return $this
                ->addUsingAlias(FileTableMap::COL_NOTE_ID, $note->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(FileTableMap::COL_NOTE_ID, $note->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
