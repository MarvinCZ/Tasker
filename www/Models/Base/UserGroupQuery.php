 ...
 * @method     ChildUserGroupQuery orderByRights($order = Criteria::ASC) Order by the rights column
 *
 ...
 * @method     ChildUserGroupQuery groupByRights() Group by the rights column
 *
 ...
 * @method     ChildUserGroup findOneByRights(int $rights) Return the first ChildUserGroup filtered by the rights column *
 ...
 * @method     ChildUserGroup requireOneByRights(int $rights) Return the first ChildUserGroup filtered by the rights column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUserGroup[]|ObjectCollection findByRights(int $rights) Return ChildUserGroup objects filtered by the rights column
...
abstract class UserGroupQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT user_id, group_id, rights FROM user_group WHERE user_id = :p0 AND group_id = :p1';
        try {
    ...
    public function filterByRights($rights = null, $comparison = null)
    {
        if (is_array($rights)) {
            $useMinMax = false;
            if (isset($rights['min'])) {
                $this->addUsingAlias(UserGroupTableMap::COL_RIGHTS, $rights['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($rights['max'])) {
                $this->addUsingAlias(UserGroupTableMap::COL_RIGHTS, $rights['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserGroupTableMap::COL_RIGHTS, $rights, $comparison);
    }
