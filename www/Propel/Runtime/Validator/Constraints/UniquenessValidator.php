...
class UniquenessValidator extends ConstraintValidator
{
    public function validate($value, Constraint $constraint)
    {
        ...
        $modified = in_array(strtolower($constraint->groups[1]).'.'.$this->context->getPropertyName(), $this->context->getObject()->getModifiedColumns());

        if ($modified && 0 < $queryClass::create()->$filter($value)->count()) {
